...
import { Card, CardHeader, Avatar, Button, Modal, Typography, Stepper } from '@mui/material';
            ...
            <RainbowKitProvider chains={chains} theme={lightTheme()}>
                ...
                <div className="tokens">
                    ...
                    >
                        ...
                        >
                            <option value="0" style={{ padding: '5rem' }}>All Networks</option>
                            <option value="1" style={{ padding: '5rem' }}>Ethereum Mainnet</option>
                    ...
                    {filteredTokens.map((token, index) => (
                        <Card style={{
                            ...
                            <CardHeader
                                avatar={
                                    <img style={{ height: '3rem' }} src={token.logoURI} />
                                }
                                action={
                                    <Button onClick={() => { updateStep(token) }}>Add</Button>
                                }
                                title={`${token.name} (${token.symbol})`}
                                subheader={`${networks[token.chainId] || "Unsupported Network"} : ${token.address.substring(0, 6)}...${token.address.slice(-4)}`}
                            />
                    ...
                    {token && <Modal
                        open={token != null}
                        onClose={() => { setToken(null); setStep(0) }}
                    >
                        <Paper style={{
                            position: 'absolute',
                            top: '50%',
                            ...
                            <Typography variant="h6" gutterBottom>Add {token.name} to wallet</Typography>
                            <Stepper activeStep={step} orientation="vertical">
                                <Step>
                                    <StepLabel>Connect Wallet</StepLabel>
                                    <StepContent>
                                        <ConnectButton.Custom onConnectChange={({ isConnected, data }) => updateStep(token)}>
                                            {({
                                                account,
                                                chain,
                                                openAccountModal,
